...
import { useRadioGroupState ,setSelectedValue} from "@react-stately/radio";
...
function RadioGroup(props) {
  ...
  let state = useRadioGroupState(props);
    ...
    // state.setSelectedValue(props.checkedValue);
...
function Radio(props) {
  ...
  return (
    <label
      className={`inline-flex overflow-hidden space-x-2.5 items-start justify-start px-4 py-5 w-full ${
...
const FormatPicker = () => {
  return (
    <SSRProvider>
      <RadioGroup
        checkedValue="mp3"
        className="flex flex-col items-start justify-start w-full overflow-hidden border border-blue-100 divide-y divide-blue-100 rounded-md"
      >
        <Radio description="With the .mp3 extension from the MPEG codec" value="mp3">
          MP3 file format
        </Radio>
